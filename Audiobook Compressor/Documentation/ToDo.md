...
* \[ ] **(Bug)** 1C6: Implement a new setting, "Don't convert mono files to stereo," and relevant processing logic.* (Button has been implemented as Convert to Mono, but logic has not yet been implemented.)\*
...
* \[X] **(Feature)** 9C: Implement a dynamic panel whose visibility and content are controlled by the primary "Mono/Stereo" output setting.
